...
public class Recomend {
	public static void main(String[] args) throws Exception {
		BufferedReader br = new BufferedReader(new FileReader("user_click_data.txt"));//构造一个BufferedReader类来读取文件
		FileWriter fw = new FileWriter("user_click_data_result.txt");  
		...
		while((text = br.readLine())!=null){//使用readLine方法，一次读一行
			...
			strs[3] = strs[3].trim();
			reader = new StringReader(strs[3]);
	        ...
	        s = "";
	        strs[4] = strs[4].trim();
			reader = new StringReader(strs[4]);
			ts = analyzer.tokenStream("", reader);  
	        term=(CharTermAttribute) ts.getAttribute(CharTermAttribute.class); 
	        while(ts.incrementToken()){  
	        	s += term.toString()+"|";
	           // System.out.print(s);  
	            //fw.write(s,0,s.length()); 
	        ...
	        {
	        	strs[i] += "\t";
	        	fw.write(strs[i],0,strs[i].length()); 
	        }
	        fw.write("\r\n",0,1); 
	        fw.flush(); 
	        System.out.println();
